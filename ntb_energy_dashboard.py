...
st.markdown(
    ...
    h1, h2, h3 {{ color: #0a2e4b; font-weight: 700; }}
    .top-title {{ font-size: 2.6rem; font-weight: 800; background: linear-gradient(135deg, #0f4c75, #3282b8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 0.2rem; }}
    .top-subtitle {{ color: #1e4a6b; font-size: 1.1rem; margin-bottom: 1.2rem; font-weight: 400; }}
   ...
   @keyframes rotateInsight {{
    ...
    .stButton button {{ background: linear-gradient(135deg, #0f4c75, #3282b8); color: white; border: none; border-radius: 40px; padding: 10px 28px; font-weight: 600; box-shadow: 0 4px 12px rgba(15,76,117,0.3); }}
    </style>
    """,
...
project_data = pd.DataFrame({
    "Project": ["Lombok Peaker", "Sedau Kumbi", "Lombok FTP-2", "Lunyuk Solar", "Medang Solar", "Dedieselisasi Solar", "Sumbawa-2", "Kokok Babak", "Sumbawa-Bima Solar", "Sumbawa-Bima Biomass", "Sumbawa-Bima Geothermal", "Lombok 3 EBT Base"],
    "Technology": ["Gas", "Hydropower", "Coal", "Solar PV", "Solar PV", "Solar PV", "Gas", "Hydropower", "Solar PV", "Biomass", "Geothermal", "Renewable Base"],
    "Capacity (MW)": [10, 1.3, 100, 2, 0.3, 8.4, 30, 2.3, 10, 10, 10, 100],
    "Status": ["Operational", "Operational", "Under construction", "Plan", "Operational", "Plan", "Bidding", "Under construction", "Plan", "Plan", "Plan", "Plan"],
    "Developer": ["PLN", "IPP", "PLN", "PLN", "PLN", "IPP", "PLN", "IPP", "IPP", "IPP", "PLN", "PLN"],
    "Location": ["Lombok", "Lombok", "Lombok", "Lunyuk / Isolated System", "Medang / Isolated System", "Isolated System", "Sumbawa", "Lombok", "Sumbawa-Bima", "Sumbawa-Bima", "Sumbawa-Bima", "Lombok"],
...
color_map = {
    "Solar PV": "#f3c742", "Wind": "#8b5cf6", "Hydropower": "#3b82f6", "Bioenergy": "#7cb342",
    "Biomass": "#7cb342", "Biomass / Cofiring": "#7cb342", "Geothermal": "#ef4444", "HSD / Diesel": "#2c7fb8",
    "MFO": "#64748b", "Biodiesel": "#14b8a6", "Coal": "#475569", "Natural Gas": "#38bdf8", "Gas": "#38bdf8", "Renewable Base": "#22c55e"
}


def get_folium_color(tech: str) -> str:
    return {
        "Solar PV": "orange", "Wind": "purple", "Hydropower": "blue", "Biomass": "green",
...
page_label = {"situasi": t("nav_situasi"), "masterplan": t("nav_masterplan"), "datacenter": t("nav_datacenter")}
page = st.radio("Main navigation", page_options, format_func=lambda x: page_label[x], horizontal=True, label_visibility="collapsed")
st.markdown("---")
...
# =========================================================
    ...
    with left_col:
        ...
        st.download_button(label=t("download_project_list"), data=project_data.drop(columns=["lat", "lon"]).to_csv(index=False), file_name="ntb_project_pipeline.csv", mime="text/csv")
        st.download_button(label=t("download_pathway_data"), data=capacity_pathway_data.to_csv(index=False), file_name="ntb_nze_capacity_pathway.csv", mime="text/csv")

    with right_col:
        st.markdown(f"### {t('project_map')}")
        tech_filter = st.multiselect(t("filter_tech"), options=sorted(project_data["Technology"].unique()), default=sorted(project_data["Technology"].unique()))
